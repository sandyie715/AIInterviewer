...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (statusData.status === "expired") {
            document.body.innerHTML = '<div style="height: 100vh; display: flex; align-items: center; justify-content: center; background: #030712; color: white; text-align: center;"><div><div style="font-size: 40px; margin-bottom: 20px;">⏰</div><h2>Interview window has closed</h2><p style="color: #94a3b8; margin-top: 10px;">Please contact your recruiter to reschedule</p></div></div>';
            return;
        }
...
function showWaitingScreen(startTime, startTimeIST) {
    ...
    if (startTimeIST) {
        document.getElementById('scheduledTimeDisplay').textContent = startTimeIST;
    }
...
async function getEvaluation() {
    try {
        ...
        document.getElementById('loadingFeedback').innerHTML = '<p style="color: var(--danger);">Unable to retrieve scores</p>';
    }
}
    ...
    document.body.innerHTML = `
        <div style="height: 100vh; display: flex; align-items: center; justify-content: center; background: #030712; color: white; text-align: center;">
            <div style="max-width: 400px;">
                ...
                <h2>${message}</h2>
                <button onclick="location.reload()" style="margin-top: 25px; padding: 10px 20px; border-radius: 8px; border: none; background: white; cursor: pointer; font-weight: 700;">Retry</button>
            </div>
        </div>
    `;
}